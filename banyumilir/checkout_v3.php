      ...
      <div class="row g-0">
        <div class="col-md-8">
          ...
          <div id="location" class="p-3 bg-white border rounded-normal mb-3" hidden>
            ...
            <div id="map" class="overflow-hidden" style="height: 350px" ></div>
          </div>
...
<script>
    ...
    function selectAddress(place) {
        ...
        if (lat >= bounds[0][0] && lat <= bounds[1][0] && lon >= bounds[0][1] && lon <= bounds[1][1]) {
            ...
            alert('Address is not at Bali');
            // Tindakan tambahan jika alamat tidak valid, misalnya reset marker atau tampilkan pesan kesalahan
        }
    }

    function customRouteRenderer(route, options) {
      const renderer = L.Routing.renderer(route, options);

      // Override fungsi `createInstructions` untuk mengembalikan elemen kosong
      renderer.createInstructions = function() {
          return L.DomUtil.create('div', 'leaflet-routing-instructions');
      };
    ...
    function drawRoute(startLat, startLon, endLat, endLon) {
        ...
        routeControl.on('routesfound', function(e) {
            ...
            routes.forEach(function(route, index) {
                ...
                document.getElementById("jarak").value = (route.summary.totalDistance / 1000)+1;
            });
        });
    }
    ...
    map.on('click', function(e) {
        ...
        if (lat >= bounds[0][0] && lat <= bounds[1][0] && lon >= bounds[0][1] && lon <= bounds[1][1]) {
            ...
            alert('Only Bali`s Island avaliable.');
        }
    });
